...
const makeDeptStatus = (status: Status, by = "System", date = "2026-02-20", remarks = "", rev = 1, delay?: string): DepartmentStatus => ({
  status, updatedBy: by, updatedDate: date, remarks, revisionNumber: rev, delayReason: delay,
});
...
const projects: Project[] = [
  {
    ...
    priority: "high", deliveryDate: "2026-04-15", overall: "in-progress",
    ...
    tender: makeDeptStatus("completed", "Amit", "2026-01-10"), design: makeDeptStatus("in-progress", "Priya", "2026-02-15"),
    drawing: makeDeptStatus("pending", "Priya", "2026-02-18"), qap: makeDeptStatus("not-started"),
    bomValidation: makeDeptStatus("not-started"), purchase: makeDeptStatus("not-started"),
    production: makeDeptStatus("not-started"), finishing: makeDeptStatus("not-started"),
    dispatch: makeDeptStatus("not-started"), finance: makeDeptStatus("in-progress", "Sanjay", "2026-01-20"),
    createdBy: "Admin", createdDate: "2026-01-10", lastModifiedBy: "Priya Sharma", lastModifiedDate: "2026-02-15", versionNumber: 5,
  },
  {
    ...
    priority: "medium", deliveryDate: "2026-03-20", overall: "in-progress",
    ...
    tender: makeDeptStatus("completed"), design: makeDeptStatus("completed"),
    drawing: makeDeptStatus("completed"), qap: makeDeptStatus("completed"),
    bomValidation: makeDeptStatus("completed"), purchase: makeDeptStatus("completed"),
    production: makeDeptStatus("in-progress", "Deepak", "2026-02-10"),
    finishing: makeDeptStatus("not-started"), dispatch: makeDeptStatus("not-started"),
    finance: makeDeptStatus("in-progress"),
    createdBy: "Admin", createdDate: "2025-12-20", lastModifiedBy: "Deepak Singh", lastModifiedDate: "2026-02-10", versionNumber: 12,
  },
  {
    ...
    priority: "critical", deliveryDate: "2026-03-01", overall: "delayed",
    ...
    tender: makeDeptStatus("completed"), design: makeDeptStatus("completed"),
    drawing: makeDeptStatus("completed"), qap: makeDeptStatus("completed"),
    bomValidation: makeDeptStatus("completed"), purchase: makeDeptStatus("delayed", "Amit", "2026-02-01", "Vendor delay", 2, "Raw material shortage"),
    production: makeDeptStatus("not-started"), finishing: makeDeptStatus("not-started"),
    dispatch: makeDeptStatus("not-started"), finance: makeDeptStatus("pending"),
    createdBy: "Admin", createdDate: "2025-11-15", lastModifiedBy: "Amit Patel", lastModifiedDate: "2026-02-01", versionNumber: 8,
  },
  {
    ...
    priority: "medium", deliveryDate: "2026-05-10", overall: "in-progress",
    ...
    tender: makeDeptStatus("completed"), design: makeDeptStatus("in-progress"),
    drawing: makeDeptStatus("not-started"), qap: makeDeptStatus("not-started"),
    bomValidation: makeDeptStatus("not-started"), purchase: makeDeptStatus("not-started"),
    production: makeDeptStatus("not-started"), finishing: makeDeptStatus("not-started"),
    dispatch: makeDeptStatus("not-started"), finance: makeDeptStatus("pending"),
    createdBy: "Admin", createdDate: "2026-01-05", lastModifiedBy: "Priya Sharma", lastModifiedDate: "2026-02-12", versionNumber: 3,
  },
  {
    ...
    priority: "low", deliveryDate: "2026-06-01", overall: "not-started",
    ...
    tender: makeDeptStatus("in-progress"), design: makeDeptStatus("not-started"),
    drawing: makeDeptStatus("not-started"), qap: makeDeptStatus("not-started"),
    bomValidation: makeDeptStatus("not-started"), purchase: makeDeptStatus("not-started"),
    production: makeDeptStatus("not-started"), finishing: makeDeptStatus("not-started"),
    dispatch: makeDeptStatus("not-started"), finance: makeDeptStatus("not-started"),
    createdBy: "Admin", createdDate: "2026-02-01", lastModifiedBy: "Admin", lastModifiedDate: "2026-02-01", versionNumber: 1,
  },
  {
    ...
    priority: "medium", deliveryDate: "2026-02-28", overall: "completed",
    ...
    drawing: makeDeptStatus("completed"), qap: makeDeptStatus("completed"),
    bomValidation: makeDeptStatus("completed"), purchase: makeDeptStatus("completed"),
    production: makeDeptStatus("completed"), finishing: makeDeptStatus("completed"),
    dispatch: makeDeptStatus("completed"), finance: makeDeptStatus("completed"),
    createdBy: "Admin", createdDate: "2025-10-15", lastModifiedBy: "Sanjay Mehta", lastModifiedDate: "2026-02-25", versionNumber: 22,
  },
  {
    ...
    priority: "high", deliveryDate: "2026-04-25", overall: "delayed",
    ...
    tender: makeDeptStatus("completed"), design: makeDeptStatus("delayed", "Priya", "2026-02-05", "Client revision", 3, "Scope change"),
    drawing: makeDeptStatus("not-started"), qap: makeDeptStatus("not-started"),
    bomValidation: makeDeptStatus("not-started"), purchase: makeDeptStatus("not-started"),
    production: makeDeptStatus("not-started"), finishing: makeDeptStatus("not-started"),
    dispatch: makeDeptStatus("not-started"), finance: makeDeptStatus("in-progress"),
    createdBy: "Admin", createdDate: "2025-12-01", lastModifiedBy: "Priya Sharma", lastModifiedDate: "2026-02-05", versionNumber: 7,
  },
];
...
export default function ProjectsPage() {
  ...
  const columns = [
    { key: "projectId", header: "Project ID", render: (p: Project) => <span className="font-medium text-primary">{p.projectId}</span> },
    { key: "jobId", header: "Job ID", render: (p: Project) => <span className="font-mono text-xs">{p.jobId}</span> },
    { key: "tagId", header: "Tag ID", render: (p: Project) => <span className="font-mono text-xs">{p.tagId}</span> },
    { key: "customer", header: "Customer", render: (p: Project) => p.customerName },
    ...deptColumns.map((d) => ({
      key: d.key,
      header: d.label,
      ...
      render: (p: Project) => {
        ...
        return <StatusBadge status={dept.status}>{statusShort[dept.status]}</StatusBadge>;
      },
    })),
    { key: "priority", header: "Priority", render: (p: Project) => <PriorityBadge priority={p.priority} /> },
    { key: "delivery", header: "Delivery", render: (p: Project) => <span className="text-muted-foreground text-sm">{p.deliveryDate}</span> },
    { key: "overall", header: "Overall", render: (p: Project) => <StatusBadge status={p.overall}>{statusLabels[p.overall]}</StatusBadge> },
  ];
    ...
    <div className="space-y-5 animate-fade-in">
            ...
            <TabsContent value="customer" className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder="Linked to Customer Master" />
                </div>
                <div className="space-y-2">
                  <Label>Customer Name</Label>
                  <Input placeholder="Reliance Industries" />
                </div>
                <div className="space-y-2">
            ...
            <TabsContent value="financial" className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="INV-2026-XXXX" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2 col-span-2">
                  ...
                  <Textarea placeholder="Any financial notes..." rows={2} />
                </div>
              </div>
            </TabsContent>
          </Tabs>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete project <strong>{deleteConfirm}</strong>? This action cannot be undone.
          </p>
          <div className="flex justify-end gap-3 mt-4">
            <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 rounded-md border text-sm font-medium hover:bg-muted">
              Cancel
            </button>
            <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 bg-destructive text-destructive-foreground rounded-md text-sm font-semibold hover:opacity-90">
              Delete
            </button>
          </div>
        </DialogContent>
